...
def main(page: ft.Page):
    ...
    base_duration = 25 * 60  # Tempo base configurado
    duration = base_duration  # Tempo que vai diminuindo
    running = False
    ...
    def finish_cycle(e=None):
        ...
        save_to_history(format_time(base_duration))
        duration = base_duration
        time_left.value = format_time(duration)
        play_sound("sound.mp3")
        page.update()

    # Função para configurar tempo personalizado
    def set_custom_time(e):
        nonlocal duration, base_duration
        try:
            minutes = int(custom_minutes.value)
            if minutes <= 0:
                raise ValueError
            base_duration = minutes * 60
            duration = base_duration
            time_left.value = format_time(duration)
    ...
    def set_timer(duration_minutes):
        nonlocal base_duration, duration
    ...
    def show_history(e):
        ...
        page.add(
            ft.Column([
                ft.Row([
                    ft.Text("Histórico", size=30, weight="bold"),
                    ft.IconButton(icon=ft.Icons.BRIGHTNESS_6, on_click=toggle_theme)
                ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                ft.Column(history_list, scroll=ft.ScrollMode.AUTO, expand=True, alignment=ft.MainAxisAlignment.CENTER),
                ft.Row([
